    ...
    >
      ...
      <div className="p-4 border-b border-white/50">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h3 className={clsx("font-bold text-lg", definition.color)}>
            {definition.label}
          </h3>
        </div>
        <p className={clsx("text-sm opacity-80 pl-1", definition.color)}>
      ...
      <div className="flex-1 p-4 min-h-[160px] space-y-2">
        {items.length === 0 && !isOver && (
          <div className="h-full flex items-center justify-center border-2 border-dashed border-white/40 rounded-lg">
            <p className={clsx("text-sm font-medium opacity-50", definition.color)}>
              拖曳卡片到這裡
            </p>
          </div>
        )}
        ...
        {items.map((item) => (
          <div
            key={item.id}
            className="flex items-center gap-2 p-3 bg-white/80 backdrop-blur-sm rounded-lg shadow-sm animate-pop"
          >
            <CheckCircle2 className={clsx("w-4 h-4", definition.iconColor)} />
            <span className={clsx("font-medium", definition.color)}>
              {item.label}
            </span>
          </div>
        ))}
