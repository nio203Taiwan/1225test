...
export default function App() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (item.type === targetBucketType) {
      ...
      if (poolItems.length === 1) setCompleted(true);
    } else {
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={resetGame} className="p-2 text-slate-500 hover:bg-slate-100 rounded-full transition-colors"><RefreshCw className="w-5 h-5" /></button>
          </div>
        </div>
      </header>
        ...
        <main className="flex-1 max-w-6xl mx-auto w-full p-4 md:p-6 grid lg:grid-cols-4 gap-6 md:gap-8">
          <section className="lg:col-span-1 order-2 lg:order-1">
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm h-full">
              <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">待分類卡片 ({poolItems.length})</h2>
              <div className="space-y-2">
                {poolItems.map((item) => (
                  <DraggableItem key={item.id} item={item} isShaking={shakeItemId === item.id} />
                ))}
                {poolItems.length === 0 && !completed && <div className="text-center py-10 text-slate-400 text-sm">完成分類！</div>}
              </div>
            </div>
          </section>
          <section className="lg:col-span-3 order-1 lg:order-2 grid md:grid-cols-3 gap-4">
            {BUCKET_DEFINITIONS.map((def) => (
              <Bucket key={def.id} definition={def} items={buckets[def.id]} />
            ))}
          </section>
        </main>
        <DragOverlay>
      ...
      {completed && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center shadow-2xl animate-pop">
            ...
            <h2 className="text-2xl font-bold mb-2">挑戰成功！</h2>
            <p className="text-slate-600 mb-8">你已經掌握了這個實驗的變因設計。</p>
            <div className="flex gap-3 justify-center">
              <button onClick={resetGame} className="px-6 py-3 rounded-xl font-bold text-slate-600 hover:bg-slate-100 transition-colors">再玩一次</button>
              <button onClick={nextLevel} className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center gap-2">下一關<ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
      )}
